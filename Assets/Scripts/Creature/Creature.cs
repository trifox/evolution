...
namespace Keiwando.Evolution
{
    ...
    public class Creature : MonoBehaviour
    {
        ...
        public float GetDistanceFromObstacleXMinus(GameObject obstacle)
        {
            ...
            for (int i = 0; i < distances.Length; i++)
            {
                if (joints[i].center.x > obstaclePos.x &&
                Vector3.Distance(joints[i].center, obstaclePos) < minDistance)
                {
                    minDistance = Vector3.Distance(joints[i].center, obstaclePos);
                }
            }
            return minDistance;
        }
        public float GetDistanceFromObstacleXPlus(GameObject obstacle)
        {
            ...
            for (int i = 0; i < distances.Length; i++)
            {
                if (joints[i].center.x < obstacle.transform.position.x &&
                Vector3.Distance(joints[i].center, obstaclePos) < minDistance)
                {
                    minDistance = Vector3.Distance(joints[i].center, obstaclePos);
                }
        ...
        public CreatureStats GetStatistics(int simulationTime)
        {
            ...
            stats.unclampedFitness = fitness*fitness;// use squared fitness
